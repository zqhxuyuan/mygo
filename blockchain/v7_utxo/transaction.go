...
func (tx Transaction) IsCoinbase() bool {
	...
	//在NewCoinbaseTX方法中，创建的一个TXInput{[]byte{}, -1, nil,}, 第一个参数是txid, 第二个参数是Vout
	//所以一个Transaction如果只有一个TXInput，并且txid的长度等于0, Vout=-1，它就是coinbase transaction
	//由于coinbase实际上也只有一个TXOutput，所以还可以加上限制条件：len(tx.Vout)==1
...
func (tx *Transaction) Verify(prevTXs map[string]Transaction) bool {
	...
	for inID, vin := range tx.Vin {
		// 下面的逻辑和签名的一样
		prevTx := prevTXs[hex.EncodeToString(vin.Txid)]
		fmt.Println("prevTX vout len :", len(prevTx.Vout))
		fmt.Println("vin's Vout index:", vin.Vout)
		txCopy.Vin[inID].Signature = nil
		txCopy.Vin[inID].PubKey = prevTx.Vout[vin.Vout].PubKeyHash
		txCopy.ID = txCopy.Hash()
		txCopy.Vin[inID].PubKey = nil
